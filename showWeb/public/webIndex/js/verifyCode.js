...
(function(){
    var Code = (function(){
        ...
        var init = function(){
            W = 393;
            H = 100;
            var imgCodeEle = document.getElementById("imgCode");
            ...
            imgCodeEle.appendChild(codeContainer);
        ...
        var getPics = function(){
            var codeCount = Math.round(Math.random()*(codeInfo.count<5?codeInfo.count:5)),itemArr = [],extraArr = [],pics = [];
            codeCount = codeCount ? codeCount: codeCount + 1;
            var extraCount = 5 - codeCount;
            for(var i=0;i<codeInfo.count;i++){
                var src = "/webIndex/images/codeImgs/"+ codeInfo.name + "/" + (i+1) +".jpg";
                var im  = new Image();
                im.src = src;
                itemArr.push({
                    'img' : im,
            ...
            for(var index in sources){
                ...
                if(item.name!=codeInfo.name){
                    for(var i=0;i<item.count;i++){
                        var src = "/webIndex/images/codeImgs/"+ item.name + "/" + (i+1) +".jpg";
                        var im  = new Image();
                        im.src = src;
                        extraArr.push({
                            'img' : im,
        ...
        var particlePics = function(pics){
            ...
            for(var i=0,l=pics.length;i<l;i++){
                (function(i){
                    var item = pics[i],im = item.img,x = (i%5)*(picWidth+gap),y=((i/5)>>0)*(picWidth+gap)+35;
                    if(item.name===codeInfo.name){
                        vCode.push({
                            'x' : x,
                            'y' : y
